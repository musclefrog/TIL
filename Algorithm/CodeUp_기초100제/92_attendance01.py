...
n = int(input()) # 출석을 부르는 횟수
a = input().split() # 해당 번호를 부름.

for i in range(n): # a를 정수형으로 변환
    a[i] = int(a[i])
    
d = [] # 빈 리스트 변수
for i in range(24): # 리스트 d에 0을 넣음
    d.append(0)
    
for i in range(n): # 번호를 부를 때 마다 그 번호에 대한 카운트 1씩 증가
    d[a[i]] += 1
